...
resource primary_vnet 'Microsoft.Network/virtualNetworks@2021-02-01' = {
  ...
  properties: {
    ...
    subnets: [for (subnetName, i) in subnets: {
      name: subnetName
      properties: {
        addressPrefix: (subnetName == 'containerappcontrol') ? '10.0.100.0/21' : (subnetName == 'containerapp') ? '10.0.108.0/21' : '10.0.${i}.0/24'
...
resource dr_vnet 'Microsoft.Network/virtualNetworks@2021-02-01' = {
  ...
  properties: {
    ...
    subnets: [for (subnetName, i) in subnets: {
      name: subnetName
      properties: {
        addressPrefix: (subnetName == 'containerappcontrol') ? '172.16.100.0/21' : (subnetName == 'containerapp') ? '172.16.108.0/21' : '172.16.${i}.0/24'
        serviceEndpoints: (startsWith(subnetName, 'appsvc')) ? [
          {
            service: 'Microsoft.Sql'
            locations: [
